...
11. **Auth is minimal JWT, decided 2026-07-11; self-registration closed after first account, decided 2026-07-15.** `users` table with `owner | editor | viewer` roles. `POST /auth/register` only ever succeeds when the `users` table is empty — it creates the one and only self-registered account (always `owner`) and then 403s permanently (`UsersService.register()`). This is an internal, single-org admin tool, not a public signup product — there is no standing "create account" page; every account after the first is created by an owner via `POST /users` (Members tab UI), never through `/register`. `GET /auth/setup-status` (public, `{needsSetup: boolean}`) is what the frontend uses to show the one-time setup form — `Login.tsx` redirects to `/register` when `needsSetup` is true, and `Register.tsx` redirects back to `/login` when it's false, so the form is only ever reachable during that first-run window. Don't add a persistent "Sign up" link back — that was the exact thing being closed off here. `JwtAuthGuard` + `RolesGuard` currently gate only the RBAC-scoped controllers named in GC-056 (templates/sequences/lists) — reads need any authenticated role, writes need `owner`/`editor`. Don't silently expand guard coverage to other controllers (contacts, tags, webhooks) without a ticket — GC-056 named its scope deliberately. `AuditLogService` is called from every guarded write endpoint; `GET /audit-log` is `owner`-only.
...
14. **The public API (`POST /api/v1/contacts`, decided 2026-07-17) is bearer-API-key-authenticated, not HMAC-signed** — a deliberate exception to invariant 4 above. Invariant 4 covers the generic inbound-webhook-relay framework (arbitrary field mapping, fires triggers); the public API is a purpose-built "create/upsert a contact" REST endpoint aimed at external tools (website forms, Zapier) that mostly can't compute a per-request HMAC signature, only send a static header. Keys live in `api_keys` (hash-only at rest, sha256, raw value shown once at creation — `apps/api/src/api-keys/`), auth via `ApiKeyAuthGuard` checking `X-Api-Key`, entirely separate from `JwtAuthGuard`/`RolesGuard`. Each key can carry a `defaultListId`/`defaultTagIds` so one key = one integration's fixed destination, without the caller needing to know internal ids (as of GC-123, these are no longer set from the create form — the UI only asks name + expiry — but the columns/behavior still exist for keys that have them). Key management (create/rotate/revoke, expiry defaulting to 1 year with a warning on never-expire) lives in Settings > API keys, not the Webhooks page (moved there by GC-123 since it's credential management, same trust level as Settings > Integrations). `/api/v1/*` is rate-limited (`@nestjs/throttler`, 60 req/min per key, `PublicApiModule`). See `docs/PUBLIC_API.md`. If a future public endpoint needs inbound external auth, default to this API-key pattern for simple "push data in" surfaces and to invariant 4's HMAC pattern only for the generic webhook-relay/trigger-firing framework specifically.
15. **`TRACKING_DOMAIN` is DB-only and DNS-verified, decided 2026-07-14.** Unlike every other Settings > Integrations field, it's never read from `.env` and can't be saved through the generic bulk PATCH (`SettingDef.verifyOnly: true` in `known-settings.ts` marks this). `POST /settings/tracking-domain/verify` (`tracking/tracking-domain.controller.ts`) checks a live DNS CNAME from the submitted domain to the API's own request hostname before calling `SettingsService.setMany()` — a typo'd or unowned domain can never quietly become the open/click tracking host. No local-dev bypass — the check always runs, even against `localhost`, so the "show the DNS record" step is exercisable and testable in every environment (a localhost target can never actually verify, which is correct — there's no real domain that should ever point there). If a future custom-domain feature needs the same "prove you control this DNS" pattern (e.g. a custom sending domain), follow this same shape rather than a plain env-backed field.
